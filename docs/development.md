...
2. **A preview asset** — done: `package.json` declares `pi.image` pointing at `assets/demo-server-https.gif`, the README's hero capture, and the README also shows both `pi-coder-ayu` captures. The assets live in `assets/`, which is not in the npm `files` list, so they stay out of the tarball. They are served through jsDelivr (`https://cdn.jsdelivr.net/gh/jayli/pi-coder@main/assets/...`) rather than `raw.githubusercontent.com`, which timed out intermittently from this machine; either form needs `assets/` to be on `main`. jsDelivr does not serve GitHub files over 20 MB, so anything near that size has to be compressed first — the hero capture is a 30.8 s, 1002×563, 30 fps recording encoded with `ffmpeg` (`scale=1002:563`, `palettegen`/`paletteuse` with `dither=none`) and then `gifsicle -O3 --lossy=20`, which takes it from 12.6 MB to 7.3 MB at 0.98 SSIM measured at the README's 600 px display width. To use a video instead, upload an MP4 (a `github.com/user-attachments/...` URL works) and declare `pi.video`:
