...
| `simple-task/` | 轻量任务清单（`task_set` / `task_update` / `task_get`）。计划批准后模型认为该建清单就自己 `task_set`，扩展不再代它建（2026-09-24 起与 plan-mode 无耦合）。三个工具都带 `renderShell: "self"`（用户 2026-09-26 定，与 bash / read 块同一套壳）：整块**没有底色**（pending / 成功 / 失败三色底都不画）、**没有上下边界空行与左右 padding**（都是默认壳 `Box(1, 1)` 画的），标题与结果都顶格列 0；块上方只剩 pi self 模式固定的那一行留白。形状断言见 `render.test.ts`（3 个端到端用例，含「其他工具底色照旧」的对照） |
...
- **simple-task 的三个工具也带 `renderShell: "self"`**（用户 2026-09-26 定）：与 bash / read 块同一套观感 ——
  整块没有底色、没有上下边界空行与左右 padding。它不需要 bash / read 那套自绘左边距（`withHeadBar`）：
  `renderCall` / `renderResult` 返回的本来就是 `new Text(…, 0, 0)`（零 padding、无 bgFn），self 模式下
  直接顶格列 0，不用再包 Box。`render.test.ts` 的 3 个用例过 pi 自己的加载器 + `ToolExecutionComponent`
  钉住形状（含「其他工具底色照旧」的对照断言）。
