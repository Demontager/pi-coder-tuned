...
- **`docs/handbook.zh.md`** — resynced verbatim from the snapshot README; the two-phase plan mode, the three-tier authorization, the seatbelt boundary and the retirement of `destructive-guard` are all described there in full.
...
- The suite grows from **953 to 1117 tests** (measured: 1117 tests, 1099 pass, 18 skipped, 0 fail, 37.8 s wall time with `--test-concurrency=4`). The 18 skips are the real-sandbox cases in `bash-command-collapse/render.test.ts`: nested `sandbox-exec` is unavailable inside a pi session, so they report themselves as skipped instead of faking a pass.
