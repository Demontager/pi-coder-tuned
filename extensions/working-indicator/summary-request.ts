 ...
 *
 * 只有当原文**明显**放不下时才值得花一次请求。三档：
 *
 *   原文宽度 ≤ 可用宽度            → 整段放得下，`layoutPromptSummary` 全量展示，不请求
 *   可用宽度 < 原文 ≤ 可用宽度×倍数 → 截断补 `…` 就够（只丢开头一小截），不请求
 *   原文 > 可用宽度×倍数            → 请求摘要（这就是「超过可显示区域**太多**」的「太多」）
 *
 * 倍数是 `triggerRatio`（`PI_WORKING_SUMMARY_TRIGGER`，默认 1.2）：默认口径是「稍微放不下就
 * 压」—— 截断会把近两成原文丢在那一格之外时才值得花这次请求；调到 2 就是「丢掉一半以上才
 * 压」，1 就是「只要放不下就压」。宽度一律是**可见列数**（调用方用 pi-tui 的 `visibleWidth`
 * 量好传进来），不是字符数 —— 一个汉字占 2 列，用字符数判断会把中文提示词全判成「还好」。
 *
 * ## 目标长度（`targetWidth`）
 *
 ...
 * 截断原文和它一样难读。
 *
 * **只请求一次（调用方保证，不是这里的逻辑）**：模型回来的摘要超长也**不重试**，由
 * `layoutPromptSummary` 按可见宽度截断补 `…`。重试会让这一格的内容来回跳，而且第二次
 * 大概率还是长 —— 需求原文即「不要再第二次请求了，正常做截断处理就好」。
 *
 * ## 清洗（`cleanSummaryText`）
 *
...
/**
 * 触发倍数：原文宽度超过可用宽度的这个倍数才值得请求摘要。
 * 1.2 意味着「稍微放不下就压」—— 截断会丢掉近两成原文时才值得花这次请求；
 * 2 是「丢掉一半以上才压」，1 是「只要放不下就压」。
 */
export const DEFAULT_TRIGGER_RATIO = 1.2;
